/*
   ...
   The file 'hq_driver.cpp' is part of ERT - Ensemble based Reservoir Tool.
...
static auto logger = ert::get_logger("job_queue.hq_driver");

struct HQJob {
    HQJob(int job_id) : job_id(job_id), string_id(std::to_string(job_id)) {}
...
#define HQ_DRIVER_TYPE_ID 70443215
#define DEFAULT_HQ_SUBMIT_CMD "hq" // hq submit
#define DEFAULT_HQ_CANCEL_CMD "scancel"  // hq job cancel
#define DEFAULT_SQUEUE_CMD "squeue"  //  hq job list ?
#define DEFAULT_SCONTROL_CMD "scontrol" // ?
#define DEFAULT_SQUEUE_TIMEOUT 10

#define SLURM_PENDING_STATUS "PENDING" // Waiting
#define SLURM_COMPLETED_STATUS "COMPLETED" // Finished
#define SLURM_RUNNING_STATUS "RUNNING" // Running
#define SLURM_FAILED_STATUS "FAILED" // Failed
#define SLURM_CANCELED_STATUS "CANCELLED" // Cancelled
#define SLURM_COMPLETING_STATUS "COMPLETING" // ?
#define SLURM_CONFIGURING_STATUS "CONFIGURING" // ?

struct hq_driver_struct {
...
UTIL_SAFE_CAST_FUNCTION(hq_driver, HQ_DRIVER_TYPE_ID);
static UTIL_SAFE_CAST_FUNCTION_CONST(hq_driver, HQ_DRIVER_TYPE_ID);

void *hq_driver_alloc() {
    hq_driver_type *driver = new hq_driver_type();
    UTIL_TYPE_ID_INIT(driver, HQ_DRIVER_TYPE_ID);
    driver->sbatch_cmd = DEFAULT_HQ_SUBMIT_CMD;
    driver->scancel_cmd = DEFAULT_HQ_CANCEL_CMD;
...
void hq_driver_free(hq_driver_type *driver) { delete driver; }

void hq_driver_free__(void *__driver) {
    hq_driver_type *driver = hq_driver_safe_cast(__driver);
    hq_driver_free(driver);
}

const void *hq_driver_get_option(const void *__driver,
                                    const char *option_key) {
    const hq_driver_type *driver = hq_driver_safe_cast_const(__driver);
...
*/
static std::string make_submit_script(const hq_driver_type *driver,
    ...
    fprintf(submit_stream, "#HQ --cpus=%d\n", num_cpu);
    fprintf(submit_stream, "#HQ --stdout=%s.stdout\n", job_name);
    fprintf(submit_stream, "#HQ --stderr=%s.stderr\n", job_name);
    /*
...
void *hq_driver_submit_job(void *__driver, const char *cmd, int num_cpu,
    ...
    std::vector<std::string> sbatch_argv = {
        "submit",
        "--cwd=" + std::string(run_path),
        "--directives=file",
        "--name=" + std::string(job_name)};
    /*
    ...
    */
    ...
    return new HQJob(job_id);
}

static job_status_type
hq_driver_translate_status(const std::string &status_string,
...
load_scontrol(const hq_driver_type *driver, const std::string &string_id) {
    auto file_content =
...
static job_status_type
hq_driver_get_job_status_scontrol(const hq_driver_type *driver,
                                     const std::string &string_id) {
    ...
    auto status = hq_driver_translate_status(status_string, string_id);
...
hq_driver_get_job_status_scontrol(const hq_driver_type *driver,
                                     int job_id) {
    return hq_driver_get_job_status_scontrol(driver, std::to_string(job_id));
}

static void hq_driver_update_status_cache(const hq_driver_type *driver) {
    ...
    while (offset != std::string::npos) {
        ...
        auto status = hq_driver_translate_status(
    ...
    for (const auto &job_id : active_jobs) {
        auto status = hq_driver_get_job_status_scontrol(driver, job_id);
...
void hq_driver_free_job(void *__job) {
    HQJob *job = static_cast<HQJob *>(__job);
    delete job;
}
